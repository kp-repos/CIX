...
def second_lab_audit(store: Store, units: list[InteractionUnit], rubric: Rubric,
                     hit_artifact_id: str, client2: ModelClient, cfg: SecondLabConfig, seed: int,
    ...
    provenance_lab=None means 'non-sibling provenance' (human-authored or unknown
    origin, e.g. a real FS corpus) and does NOT recuse — the seat proceeds. Callers
    MUST populate provenance_lab accurately for every AI-generated corpus (read it
    from the corpus provenance record); a missing tag fails OPEN by design, so the
    guard's integrity rests on the caller reading provenance, not on this function."""
