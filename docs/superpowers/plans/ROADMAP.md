...
**Progress:** ✅ G1 (deterministic spine) · ✅ G2 (thin end-to-end slice) · ✅ **G3 (calibration — holdout 6/6 T-CAL, 0/100 T-NULL, 2026-08-01)** · ✅ **G4 (assembly — capability subset, 2026-08-02; scrub + A11, catalogue/priced + A5, A9 service rubric, swap proofs AC-6/AC-7, four-layer self-test + A10, differential tooling; T-SST + T-DIFF frozen)** · ✅ **G5 rehearsal (2026-08-03; servicegen synthetic service corpus, cix self-test / cix differential CLI, end-to-end O1 dress run — the G4 follow-on's tooling half)** · ✅ **G6 demo tooling (2026-08-03; `cix query` live evidence resolution, method page, demo runbook — pipeline is O1 demo-ready; a demo-prep pass also fixed a real evidence-sampling defect where synthesis received no snippet ranges)** · ▶ **G5 next** (first real run). Three real corpora are acquired and **D-1/OD-1 is resolved** — the first real run is the **CFPB pilot** (Block, Inc. vs Bank of America, 2024+) using `configs/complaint_rubric_v1.yaml`, per `docs/superpowers/specs/2026-08-05-cfpb-comparative-briefing-design.md` and `docs/cfpb_pilot_runbook.md`. The full 63K-pair run is gated on a complaint-rubric calibration pass (not the pilot). Executed detail lives in `2026-08-02-g4-assembly.md`, `2026-08-03-g5-rehearsal-design.md`, the 2026-08-05 spec, and the PRD changelog.
...
**Core logic:** almost no new code — G5 is *execution* of machinery that has individually passed. The first real run is the **CFPB pilot** (Block, Inc. vs Bank of America, 2024+, complaint rubric) per the 2026-08-05 spec and `docs/cfpb_pilot_runbook.md`; the FS service rubric (A9) above is validation infrastructure, not the real-run gate. Three measurements come back; they are the reason the project exists:
