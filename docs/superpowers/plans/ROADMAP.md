...
**Status:** high-level overview, not an implementation plan. The G1 and G2 plans in this directory are executable; G3–G5 plans get written **after G2 lands** — their interfaces derive from G2's code, their contents from G0 corpus facts and PO-authored artifacts (rubrics, calibration spec). This document records the *logic* of each remaining gate so the shape of the whole build is visible now.
