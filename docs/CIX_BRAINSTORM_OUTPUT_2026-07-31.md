# CIX Brainstorm Output — 2026-07-31 · rev 2.4
...
**Substrate rule (rev 2.4, back-propagated from PRD v1.3 §2.3-S; ratified by KP 2026-08-05).** O3 requires a corpus whose pathology distribution arose from an actual operation, not scenario design. Four substrate classes govern which outcomes a corpus may serve: **S1** real operational (serves O1/O2/O3); **S2** real non-conversational, e.g. complaint narratives (serves O1/O2, and **O3 for corpus-level items** — distribution/frequency/rank recovery is testable on monologue data; conversation-dependent items are skipped via the corpus-property gate); **S3** roleplay / Wizard-of-Oz (O1 only); **S4** synthetic (O1 labelled + calibration tier). Roleplay, WoZ and synthetic corpora must never emit an O3 verdict — the reason is structural: where the distribution was authored, the self-test measures the authoring. Every run manifest records the substrate class, and a non-S1 corpus emits `o3: not-applicable — substrate class {n}` as hard behaviour, enforced in code, not by convention. This corrects an earlier draft that excluded S2 from O3 outright.

**Licence tier separate from substrate class (rev 2.4).** A corpus may be S1 yet non-commercially licensed — serving O2/O3 but not O1. The manifest records `licence_tier: {commercial | non-commercial | public-domain}` independently of substrate class; the release gate blocks O1 publication on any non-commercial corpus. **The NC route is approved for internal O2/O3** (KP, 2026-08-05), so the acquired CC BY-NC corpora are usable for the capability/tech-risk objective while O1 publication stays gated.

**Corpus-property gating (rev 2.4).** The manifest records `speaker_attribution` (`native`/`inferred`/`none`), `economic_signal` (`present`/`redacted`), and `ivr_structure` (`present`/`partial`/`absent`). A rubric item declaring a dependency on a property the corpus lacks is **skipped and reported as skipped** — never evaluated against absent data, never silently degraded; coverage denominators exclude skipped items.

**Speaker-attribution family (rev 2.4, PRD v1.3 R-SPK-1..3).** The normalizer records `speaker_attribution` per corpus. **KP ruling (2026-08-05): option (b), speaker-agnostic rubric for v1** — `speaker_attribution: none` is the v1 path; the whole-corpus claim (including repeat-contact chains, the highest-value observed pattern) is testable without knowing who spoke. Inference runs only as an explicit, separately keyed stage with per-turn confidence, never a silent normalizer step, and is **deferred to v1.5** (trigger: a demonstrated-value rubric item that cannot be expressed speaker-agnostically). Rubric items declare `requires_speaker`; those requiring speakers are skipped against a `none` corpus.

**Ingest hardening (rev 2.4, PRD v1.3 R-IDX-8..10).** Three corpus-specific defects each silently corrupt counts, and counts are the product: AppleDouble shadow files (`__MACOSX/`, `._*`) carrying `.json` extensions are excluded; interactions are content-hash-deduplicated before indexing with the duplicate count logged; and PII redaction tokens (`[PERSON_NAME]`, `[ORGANIZATION]`, `[MONEY_AMOUNT]`, `[DURATION]`) are treated as non-content — not tokenised as vocabulary, not rendered as if spoken, not count-inflating when repeated.

**D-1 resolution (rev 2.4).** Open decision #1 (FS corpus facts + informal-clearance record) is **resolved**: a real corpus is acquired and inspected — AIxBlock CallCenterEN, `medicare_inbound` slice, 61,513 healthcare-practice inbound conversations, median 387s — with two further corpora acquired (CFPB filtered narratives, 1.5M rows, public-domain, substrate S2, carrying a 270×-varying outcome label; Twitter Customer Support, 794K conversations, CC BY-NC-SA, substrate S1 with native speaker attribution). Facts, defects and ingest hazards are recorded in `docs/reference/CIX_Corpus_Sourcing_Memo_2026-08-03.md`. The FS corpus is no longer on the critical path. **Note (patch §C): the "Author order ≠ run order" ruling survives unchanged** — it was written for FS financial-services inbound; the acquired corpus is healthcare-practice inbound, both service-side, so the ruling holds across the substrate swap.
...
|---|---|---|---|
| 1 | FS corpus facts: volume, date range, transcript format; record informal clearance in manifest (per §10 ruling — no written gate) — **RESOLVED 2026-08-03**, corpus acquired: AIxBlock CallCenterEN + CFPB + Twitter CS (see D§10 "D-1 resolution" note and memo `docs/reference/CIX_Corpus_Sourcing_Memo_2026-08-03.md`) | PO | Before real-data ingest |
...
- **2026-08-05 — rev 2.4.** Back-propagation from PRD v1.3 (Patch v2.1, ratified by KP 2026-08-05; per §0.2 governance rule). D§10 amended with: the substrate taxonomy (S1–S4) and its structural O3 exclusion for authored distributions, with S2 corrected to serve O3 for corpus-level items; licence-tier separation from substrate class (NC route approved for internal O2/O3); corpus-property gating (`speaker_attribution`/`economic_signal`/`ivr_structure`) with skip-and-report; the speaker-attribution family (KP option (b) — `none` is the v1 path, inference deferred to v1.5); and ingest hardening (shadow-file exclusion, dedup, redaction-token awareness). D-1 recorded as resolved against the three acquired corpora (memo `docs/reference/CIX_Corpus_Sourcing_Memo_2026-08-03.md`). The "Author order ≠ run order" ruling explicitly survives the substrate swap, unchanged.
